...
* Prerequisite: Use Usim to generate data (this should be covered by [SynthDet](https://github.com/Unity-Technologies/SynthDet/blob/master/docs/RunningSynthDetCloud.md))
* Transfer this data to your own GCP project's Storage Bucket. Note that your storage bucket _must be in US-CENTRAL1 region_.

## Phase 2: Transform Data to TensorFlow format (tfrecord)

In order for a built-in model training algorithm to work with the data generated by Unity Simulation, we need to transform it into the tfrecord format. In order to do this, we'll be using GCP's DataFlow service.

* First, we'll get a subset of the data locally and test with it to make sure our transformation works.
* Then we'll run DataFlow on your own GCP project against the data bucket from phase 1 to transform the data to tfrecords.

## Phase 3: Use Google AI Platform to train an object detection model

There is a choice of using either the Google Cloud SDK (`gcloud`) or the GCP GUI to submit a model training job.
In order to use `gcloud`, we need to do the following:
* Create a configuration yaml file that defines the compute resources.
* Invoke `gcloud` with this file and some other parameters to kick off the job.
...
* DataFlow enabled for your project. More instructions can be found on Google's [official DataFlow documentation](https://cloud.google.com/dataflow/docs/quickstarts/quickstart-python). If you're an owner of the project, you'll only need to follow the guide up to Step 3 of the "Before you begin" section.
* You'll need AI Platform enabled for your Google Project. [Official guide here](https://cloud.google.com/ai-platform/training/docs/algorithms/object-detection-start). Please ensure you've done everything up to the "Setup" section of that guide.
...
1. Set up a Python virtual environment. We're assuming you're on Python3 (less than Python3.8).
   ...
   ```
2. Make an outer run executions directory manually and copy the single app param into it. Here's an example:
   ```bash
   ...
   gsutil -m cp -r <paste your path here> .
   cd ..
   ```
...
#### (Optional) Convert real world data to Tensorflow format (tfrecord)
