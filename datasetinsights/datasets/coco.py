...
class CocoDetection(Dataset):
    ...
    def _preprocess_dataset(data_path, split):
        ...
        if archive_img_file.exists() and archive_ann_file.exists():
            unarchived_img_path = CocoDetection.unarchive_data(
                data_path, archive_img_file
            )
            unarchived_ann_path = CocoDetection.unarchive_data(
    ...
    @staticmethod
    def unarchive_data(data_path, archive_file):
        file_descriptor = os.open(archive_file, os.O_RDONLY)
        try:
            fcntl.flock(file_descriptor, fcntl.LOCK_EX)
            unarchived_path = Path(data_path)
            if not CocoDetection.is_dataset_files_present(unarchived_path):
