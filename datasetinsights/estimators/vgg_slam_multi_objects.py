...
def vgg_slam_c_s(config):
    ...
    model_slam.compile(loss={'output_trans': 'mse'},
                       optimizer=adam)

    return model_slam


class VGGSlamMO(Estimator):
    ...
    def train(self, **kwargs):
        ...
        X_train, X_val, y_train, \
            y_val = self.data_loader(dataset_df)
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            logger.info(
                ...
                f"Train Loss mse : {train_trans:.3f}\n"
                f"Validation Loss mse: {val_trans:.3f}\n"
            )

            writer.add_scalar(
                "Validation/mse_loss cube", val_trans, epoch
            )
            writer.add_scalar(
    ...
    def evaluate(self, **kwargs):
        ...
        X_train, X_val, y_train, y_val = self.data_loader(val_df)
        self._evaluate_one_epoch(X_val, y_val, 1, 1)
    ...
    def save(self, epoch):
        ...
        """
        epoch = "ep" + str(epoch)
        file_name = "UR3_multi_objects_vgg_2" + epoch
