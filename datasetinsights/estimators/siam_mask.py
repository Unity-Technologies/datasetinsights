...
class AnchorTargetLayer:
    ...
    def __call__(self, anchor, target, size, neg=False, need_iou=False):
        ...
        tcx, tcy, tw, th = corner2center([target.x, target.y, target.x+target.w, target.y+target.h] )
        ...
        # print(target.x, target.y, target.x+target.w, target.y+target.h)
        # exit(0)
        # IoU
        overlap = IoU([x1, y1, x2, y2], [target.x, target.y, target.x+target.w, target.y+target.h])
        # print(overlap)
        # exit(0)
...
class SiamMask(Estimator):
    ...
    def __init__(
        self, *, config, writer, checkpointer, device, gpu=0, rank=0, train_mode=True, **kwargs
    ):
        ...
        if self.config.model_type == "3branch":
            self.model = SiamMask3Branch(anchor_cfg = config["train"]["anchors"], device = device).to(self.device)
    ...
    def build_scheduler(self, optimizer, config, epochs=50, last_epoch=-1):
        # if 'warmup' in config:
        print(config["warmup"],last_epoch)
        warmup_epoch = config['warmup']['epoch']
        sc1 = Scheduler(optimizer, config['warmup']['start_lr'],config['warmup']['end_lr'], warmup_epoch, last_epoch)
        sc2 = Scheduler(optimizer, config['start_lr'],config['end_lr'], epochs - warmup_epoch, last_epoch)
    ...
    def get_opt_lrsched(self, model, config, epoch):
        ...
        if len(backbone_feature) == 0:
            trainable_params = model.rpn_model.param_groups(config['start_lr'], key= 'mask')
        else:
        ...
        optimizer = torch.optim.SGD(trainable_params, config["warmup"]["start_lr"],
                                    momentum=config["momentum"],
                                    weight_decay=config["weight_decay"])

        lr_scheduler = self.build_scheduler(optimizer, config, epochs=config["epoch"])
    ...
    def train_loop(self,
        ...
        # path = Path("/data/savedmodels")
        path = Path(config["model_save_path"])
        path.mkdir(parents=True, exist_ok=True)

        batches_per_epoch = len(train_loader.dataset) // config["epoch"] // config["batch_size"]
        epoch = 0 
        ...
        epoch = 0
        optimizer, lr_scheduler = self.get_opt_lrsched(model.module, config, epoch)
        # optimizer, lr_scheduler = self.get_opt_lrsched(self.model, config, epoch)
        # print("LR space: ", lr_scheduler.lr_spaces)
        cur_lr = lr_scheduler.get_cur_lr()
        # ckpt = torch.load("./savedmodels/checkpoint_e0.pth", map_location=None)#lambda storage, loc: storage.cuda(self.device))
        # epoch = ckpt['epoch']
        # f = lambda x: x.split('module.', 1)[-1] if x.startswith('module.') else x
        # ckpt_model_dict = {f(key): value for key, value in ckpt['state_dict'].items()}
        ...
        for iter, input in enumerate(train_loader):
            ...
            if epoch != (iter // batches_per_epoch):
                ...
                self.save({'epoch': epoch, #,'state_dict': model.state_dict(),
                ...
                # if model.features.unfix(epoch/config["epoch"]):
            ...
            if iter % batches_per_epoch == 0 and iter != 0:
                for idx, pg in enumerate(optimizer.param_groups):
                    ...
                    writer.add_scalar('lr/group%d' % (idx+1), pg['lr'], tb_index)
            ...
            x = {
                ...
                'label_loc_weight': torch.autograd.Variable(input[4]).to(self.device),
                'label_mask': torch.autograd.Variable(input[6]).to(self.device),
                'label_mask_weight': torch.autograd.Variable(input[7]).to(self.device),
            }
            ...
            cls_weight, reg_weight, mask_weight = config['loss_weights']['feature'],config['loss_weights']['rpn'],config['loss_weights']['mask']
            ...
            if config['clip']:
                torch.nn.utils.clip_grad_norm_(model.module.features.parameters(), config['clip']['feature'])
                torch.nn.utils.clip_grad_norm_(model.module.rpn_model.parameters(), config['clip']['rpn'])
                torch.nn.utils.clip_grad_norm_(model.module.mask_model.parameters(), config['clip']['mask'])
            # if config['clip']:
            #     torch.nn.utils.clip_grad_norm_(model.features.parameters(), config['clip']['feature'])
            #     torch.nn.utils.clip_grad_norm_(model.rpn_model.parameters(), config['clip']['rpn'])
            #     torch.nn.utils.clip_grad_norm_(model.mask_model.parameters(), config['clip']['mask'])
            ...
            if (iter + 1) % 1 == 0:
                logger.info('Epoch: [{0}][{1}/{2}] lr: {lr:.6f}\t{batch_time:s}\t{data_time:s}'
                            ...
                            epoch+1, (iter + 1) % batches_per_epoch, batches_per_epoch, lr=cur_lr, batch_time=avg.batch_time,
    ...
    def train(self):
        ...
        # train_dataset.create(self.config["train"])
        # Create dataloaders
        train_loader = create_loader(train_dataset, batch_size = self.config["train"]["batch_size"])
        ...
        # self.model = torch.nn.DataParallel(self.model, list(range(torch.cuda.device_count()))).cuda()
    ...
    def evaluate(self):
        ...
        for video in videos:
            test_dataset[video] = {}
            test_dataset[video]['anno_files'] = sorted(glob.glob(join(datapath, 'Annotations/480p', video, '*.png')))
            test_dataset[video]['image_files'] = sorted(glob.glob(join(datapath, 'JPEGImages/480p', video, '*.jpg')))
            test_dataset[video]['name'] = video

        logger.info(f"Dataset DAVIS loaded (using 2016)")
        model = load_pretrain(self.model, self.config["test"]["dataset"]["saved_model"][0])
        ...
        for v_id, video in enumerate(test_dataset.keys(), start=1):
            ...
            iou_list, speed = self.track_vos(model, test_dataset[video], device=self.device,v_id = v_id)
    ...
    def track_vos(self,model,video,device,v_id):
        ...
        object_num = len(object_ids) # Always set as 1 because we are not doing multiple object tracking
        toc = 0
        ...
        for obj_id, o_id in enumerate(object_ids):
            ...
            for f, image_file in enumerate(image_files):
                ...
                if f == start_frame:  # init
                    ...
                    state = self.siamese_init(im, target_pos, target_sz, model, device=self.device)  # init tracker
                    # print(state)
                    # exit(0)
                elif end_frame >= f > start_frame:  # tracking
                    state = self.siamese_track(state, im, mask_enable=True)  # track
                    mask = state['mask']
        ...
        if len(annos) == len(image_files):
            multi_mean_iou = self.MultiBatchIouMeter(thrs, pred_masks, annos,
        ...
        if visualization:
            pred_mask_final = np.array(pred_masks)
            pred_mask_final = (np.argmax(pred_mask_final, axis=0).astype('uint8') + 1) * (
                    np.max(pred_mask_final, axis=0) > self.config["test"]["dataset"]["confidence"]).astype('uint8')
    ...
    def siamese_init(self, im, target_pos, target_sz, model, device='cpu'):
        ...
        z_crop = self.get_subwindow_tracking(im, target_pos, exemplar_size, s_z, avg_chans)
    ...
    def siamese_track(self, state, im, mask_enable=True):
        ...
        x_crop = Variable(self.get_subwindow_tracking(im, target_pos, instance_size, round(s_x), avg_chans).unsqueeze(0))
        ...
        score = FF.softmax(score.permute(1, 2, 3, 0).contiguous().view(2, -1).permute(1, 0), dim=1).data[:,
                1].cpu().numpy()
        ...
        # for Mask Branch
        # if mask_enable:
        best_pscore_id_mask = np.unravel_index(best_pscore_id, (5, score_size, score_size))
        delta_x, delta_y = best_pscore_id_mask[2], best_pscore_id_mask[1]


        mask = mask[0, :, delta_y, delta_x].sigmoid(). \
            squeeze().view(out_size, out_size).cpu().data.numpy()
        ...
        sub_box = [crop_box[0] + (delta_x - base_size / 2) * total_stride * s,
                   crop_box[1] + (delta_y - base_size / 2) * total_stride * s,
                   s * exemplar_size, s * exemplar_size]
...
class Scheduler(_LRScheduler):
    def __init__(self,optimizer,start_lr=0.03, end_lr=5e-4, epochs=50, last_epoch=-1):
        self.start_lr = start_lr
        self.end_lr = end_lr
        self.epochs = epochs
        self.lr_spaces = np.logspace(math.log10(start_lr), math.log10(end_lr), epochs)
...
class SiamMaskBase(nn.Module):
    ...
    def _add_rpn_loss(self, label_cls, label_loc, lable_loc_weight, label_mask, label_mask_weight,
        ...
        rpn_loss_loc = self.weight_l1_loss(rpn_pred_loc, label_loc, lable_loc_weight)
        rpn_loss_mask, iou_m, iou_5, iou_7 = self.select_mask_logistic_loss(rpn_pred_mask, label_mask, label_mask_weight)
    ...
    def iou_measure(self, pred, label):
        ...
        mask_sum = torch.add(pred.eq(1).type(torch.uint8),(label.eq(1).type(torch.uint8)))
